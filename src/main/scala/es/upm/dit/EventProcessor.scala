...
class EventProcessor extends RichFlatMapFunction[TrainEvent, TrainEvent] {
  ...
  private var trainMemoryState2: ValueState[TrainEvent] = _


  /*
    * Funcion de TrainEvent -> TrainEvent => No guarda array de memoria
    * Filtra los BIN
   */
  ...
  override def flatMap(input: TrainEvent, out: Collector[TrainEvent]): Unit = {
    ...
    val tmpCurrentState = trainMemoryState2.value
    ...
    if ((currentState.event_type != newState.event_type) && (currentState.date_event <= newState.date_event) && (currentState == TrainEvent("-",  "-", 0, 0, 0, "-"))) {
      ...
      trainMemoryState2.update(newMemoryTrain) //actualizamos el estado (valor actual -> valor nuevo)
  ...
  override def open(parameters: Configuration): Unit = {
    trainMemoryState2 = getRuntimeContext.getState(
      new ValueStateDescriptor[TrainEvent]("event_without_memory", createTypeInformation[TrainEvent])
    )
  }
